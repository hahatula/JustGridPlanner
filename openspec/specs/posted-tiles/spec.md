...
Turn the tiles produced by a screenshot import into locked posted grid items, merged below the local planned items, replaced on re-import (deleting the old tile files), and persisted across launches. This completes the manual-import path for posted media (`/docs/10-decisions.md` Decision 008).
